...
As a beginner range will allow to you keep your distance from Araxxor making it easier to dodge cleaves and melee hits. If you struggle with food management you may use a yak, eventually upgrading to using a nihil. The bolts equipped are Ruby bolts, but can be replaced with Dragonstone or Onyx. This is largely preference. Additionally, if desired, bolt switches can be used to scale range further in difficulty and damage.



As with most bosses, it is strongly recommended to use  |reckless|  , or your highest tier  |sharp|  if reckless is not available



.. image:: https://i.imgur.com/9AQwEMq.png



For the advanced range method it is important to have 3 things:

-  |ecb|   **Eldritch Crossbow**

-  |sgb|   |eofor|   **Essence of Finality** SGB spec stored

-  |limitless|   **Limitless ability**



Relics:

-  |heightenedsenses|  /  |berserkersfury|  Heightened Senses / Berserkers Fury
...
A combat dummy is brought for building adrenaline during the P4 transition. If you want efficient kph, use the grimoire, if you want efficient gp/hr, use scrim. If choosing to use a grim it is mainly beneficial to use during phase 2, 3 and 4. Jellies are used for healing over time without losing adrenaline.
...
.. image:: https://i.imgur.com/w037qfT.png
...
This phase is timelocked to waiting for the web to burn down. Araxxor should be lowered to ~5k life points before the web is burned down. If doing top path you can optionally use natural instinct when the web is ~80% burnt. There is not much else to this phase just dodge Araxxors specs and move to the next phase when the web burns.
...
Optionally phase right after a spec on P1. The first special on P2 is always an egg bomb, this gives you time to dps Araxxor down. Do whatever you can do deal damage quickly on phase 2. You should ideally never get a minion spawn on P2
...
Phase 1
   ...
   • Use  |NaturalInstinct|  when the web burn reaches ~70%

   • Build adrenaline to 100% and use  |DeathsSwift| 
...
Phase 2
   ...
   • Start with  |RubyBakri| 

   •  |ecb|   |spec|  →  |corruptshot|  →  |sgb|   |eofor|  →  |rapid|  →  |snap|  →  |mds| 

   •  |surge|  →  |bd|  →  |surge|  towards p3 while doing defensive abilities to gain adrenaline when araxxor does the egg bomb special attack in phase 2 (the first spec that happens in phase 2)
...
.. image:: https://i.imgur.com/QR0YouB.png
   ...
   • Run to the end of the hallway to start phase 3, then run back towards araxxor. equip ruby bolts again.

   •  |ecb|   |spec|  →  |corruptshot|  →  |rapid|  →  |snap|  /  |sgb|   |eofor|  (depending on if SGB spec is off cd yet or not)

   • By this point in the p3 rotation araxxor will start spawning minions. look out for the special spider and kill it, then finish araxxor off with bleeds and another  |snap|  /  |rapid|  as needed. Try to end phase 3 on high adrenaline.
...
Phase 1
   ...
   • Walk araxxor so he is right against the web (see <#534514775120412692>)
...
Phase 2
   ...
   • (Ramp luring/surging/natty setup from <#534514775120412692>)
 ...
 Phase 3
   ...
   •  |ecb|   |spec|  → Auto +  |sgb|   |eofor|  →  |rapid|  →   |corruptshot|  →  |frag|  + walk under →  |snipe|  →  |snap|  
...
.. image:: https://i.imgur.com/F1aXxlH.jpg
...
**Phase 3**
   ...
   • Right click attack for 2h auto +  |ecb|   |spec|  →  |corruptshot|  →  |sgb|   |eofor|  →  |snap|  →  |frag|  + walk under →  |rapid|  →  |mds| 
...
**Phase 4**
    ...
    • Spam a defensive or non-mobile surge to gain adren when cutscene ends and drop a dummy, build to 100% adren

    •  |DeathsSwift|  +  |adrenrenewal| , build with  |HydrixBakri| 

    • When Araxxi drops down onto Araxxor and is about to raise her legs, you can optionally do  |needle|  → equip ECB + switch to ruby bakriminel bolts to get the needle strike buff on most abilities on your phase 4

    • Just before Araxxi becomes attackable,  |ecb|   |spec|  → auto +  |sgb|   |eofor|  →  |limitless|   |rapid|  →  cancel with  |snap|  when araxxi has 26k hp left to avoid adrenaline drain when acid core comes out. Finish with basics if needed





**Note:** if you get a really good phase 3 SGB spec and it is still on cooldown when phase 4 becomes attackable then you may switch your rotation:

    •  |ecb|   |spec|  →  |rapid|  →  |sgb|   |eofor|  →  |snap|  →  |corruptshot|  → good basics until araxxi dies





Alternative Top Path Strategy (Courtesy of <@213693069764198401)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The idea of this method is to solo skip phase 3. If that fails then you still have  |DeathsSwift|  to help carry the damage to finish phase 3 the normal way. This method doesn't get average times as fast as the non-skip method mentioned above, but it has a way faster killtime potential due to being able to solo skip.
...
-  |dbow|   |eofspec|  **A second EoF** (Dark Bow spec)

-  |accel|  **Powerburst of acceleration**

-  |dummy|  **A second dummy**

-  |heightenedsenses|  **Heightened senses relic is mandatory**



**Phase 1:**

•  |incend|  at ~62% web burn

•  |NaturalInstinct|  at ~85% web burn

• Build to 110% adren

• Turn towards web, and when it burns, drink the powerburst of acceleration and  |surge|  →  |bd|  →  |surge|  →  |surge|  →  |bd|  straight to phase 3, then immediately drop a  |dummy|  and  |DeathsSwift| , then build adrenaline with basics and defensives using  |HydrixBakri| 
...
• Near 100% adren, use  |needle|  and switch to ruby bakriminel bolts, then use  |ecb|   |spec|  +  |adrenrenewal| . If you have extra time before araxxor catches up to you, you can also do  |anti| 

• Target araxxor and use an Auto +  |sgb|   |eofor|  → equip Dark Bow EoF then use |dbow|   |eofspec|  →  |mds| 

• If Araxxor doesn't die from that, he will start healing hp so do your best to improvise with basics while building up to  |rapid|  or  |snap|  to finish him off
...
`Top Path` <https://youtu.be/87yGonA10vA>





`Middle Path` <https://youtu.be/hMWMjMK3ePM>





`Bottom Path` <https://youtu.be/HBnVG01GKsQ>
