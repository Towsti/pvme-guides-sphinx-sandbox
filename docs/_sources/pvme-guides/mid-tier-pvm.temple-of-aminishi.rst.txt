...
.. |LotD| image:: https://cdn.discordapp.com/emojis/566453486913323042.png?v=1
    :width: 1.375em
    :height: 1.375em
...
.. |armourspike| image:: https://cdn.discordapp.com/emojis/756235792053174332.png?v=1
    :width: 1.375em
    :height: 1.375em
...
.. |DeflectRange| image:: https://cdn.discordapp.com/emojis/544195488317046812.png?v=1
    :width: 1.375em
    :height: 1.375em
...
.. |DeflectMage| image:: https://cdn.discordapp.com/emojis/544195487926845462.png?v=1
    :width: 1.375em
    :height: 1.375em
...
.. |nat| image:: https://cdn.discordapp.com/emojis/535541258131865633.png?v=1
    :width: 1.375em
    :height: 1.375em
...
.. |smoketend| image:: https://cdn.discordapp.com/emojis/583429557672083505.png?v=1
    :width: 1.375em
    :height: 1.375em
...
Introduction (Credit to <@!281823193742835714)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. image:: https://i.imgur.com/Ewd7OE8.png



**Magic (experienced/solo)**

- Useful items for Seiryu:  |DTB| 
...
.. image:: https://i.imgur.com/3Z1iimV.png



**Ranged (beginner/group)**

.. image:: https://i.imgur.com/88sDPyf.png



**Ranged (experienced/solo)**

- Useful items for Seiryu:  |LotD|   |stickybomb| 
...
.. image:: https://i.imgur.com/YXIoRgl.png



**Melee (beginner/group)**

.. image:: https://i.imgur.com/jVliUwq.png



**Melee (experienced/solo)**

- Useful items for Seiryu:  |DTB|   |armourspike| 
...
.. image:: https://i.imgur.com/2sX87wo.png





Routing Through ED1 (Map + Explanation)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. image:: https://i.imgur.com/W7030su.png



🔴 indicates the route taken

🔵 indicates what monsters should be killed
...
Room 1 - Run east until you are able to  |gbarge|  into the northernmost of the first Elite Sotapannas, then  |bd|  next to the northern barrier, walk south to lure the 4x Elite Sotapanna together and drop 2  |dommine| . Use  |assault|  (bled) followed by AoEs (save  |meteorstrike|  for later).



Room 2 - Once the first barrier is cleared Run/ |surge| / |bd|  all the way east to the area with 9x Elite Sotapanna. These enemies use Melee, so have them gather around you and use  |meteorstrike| , followed by whatever AoEs you deem necessary to kill all of the 9 afromentioned enemies and unlock the next barrier.
...
.. image:: https://i.imgur.com/Deiql3G.png
...
Room 1 -  |surge|  +  |bd|  as shown on picture: https://i.imgur.com/dZYWf7E.png

After  |bd|  stay in place until sotapanna #2 moves 1 tile west, then walk 2 tiles east from  |bd|  spot.

If done just in time then sotapanna #1 will be middle target for  |mechchin| , #2 if late.

If lured correctly, they should stand like this: https://i.imgur.com/4J70MjN.png

While waiting, use  |corruptshot|  on any sotapanna (ideally with target cycle to avoid moving in case of missclick) then  |snipe|  → q  |snap|  on middle sotapanna. Target cycle+basic to check whether any of them survived.



Room 2 - After opening gate, get a target on any of the 2 sotapannas behind it and get to 100% adren.

 |accel|  →  |bd|  + surge to wall with 9 sotapannas and use  |incend|  + click under yourself to stall it, equip  |mechchin|  → release incend shot on approx. middle target with  |corruptshot|  →  |needle| . This should kill all of the 9 sotapannas.



Room 3 -  |bd|  and walk to middle of next gate and target middle of the 3 sakadagami, wait to release your non combat spell with long range weapon then  |ricochet|  + run west for 1 tick and run back east (this should lure them together)> equip  |mechchin|  →  |bindingshot|  >  |snipe|  → q  |snap| . Finish with basics if needed and continue to next section.
...
Room 1 -  |anti|  while getting into  |gbarge|  range of the enemies.  |gbarge|  into the Elite Sotapanna (standing in the centre) and prioritise finishing him off quickly, as he is able to heal himself and the other enemies nearby. Your  |dommine|  should be off cooldown by now, so place 2 of them here. AoE down the entire room to unlock the barrier.



Room 2 - Go to either side of the room (doesn't matter which) and kill the 3x Elite Sakadagami on the side you chose.  |meteorstrike|  should be off cooldown by now, so you should use that followed by other AoEs.



Room 3 - Run/ |surge| / |bd|  all the way to the southern end of the room and kill 3x Elite Sakadagami on either side of the area. After the barrier is unlocked, teleport to the PvM hub to avoid tanking all of the Arhats/Anagami. At the PvM hub, bank if you wish to,  |DBA|  |spec|  if you wish to, get 100% adrenaline via crystal, and return to the dungeon to kill The Sanctum Guardian.

.. image:: https://i.imgur.com/9kTcwBw.png
...
Room 1 - In next section use  |anti|  immediately + start running around corner, then  |surge|  during gcd and  |ricochet|  with long range weapon sakadagami targeted in picture: https://i.imgur.com/zcs9BlH.png

After you release  |ricochet| , run north 2 tiles and use  |corruptshot|  → equip  |mechchin|  →  |dbow|  |eofspec|  if high adren, otherwise move closer and use  |rapid| . Finish rest with basics/ thresholds as needed while prioritizing killing sakadagami with low hp first as those can get healed. Make sure to end on high adren. 



Room 2 - Stand on either spot shown in picture: https://i.imgur.com/aAQI4pY.png

 |ricochet|  middle one of the 3 sakadagami on your chosen side and wait until they group up (3 ticks). Equip  |mechchin|  → use  |eofspec|  |dbow|  if high adren,  |shadowtend|  if low, finish with basics. Continue to last gate before Sanctum while building on anagamis and arhats. As you walk onto the last path with sakadagamis, equip  |mechchin|  → use  |corruptshot|  on middle sakadagami. Wait to run into distance and once your character releases  |corruptshot| , use  |tight|  (or  |bindingshot|  if low adren)>  |snipe|  → q  |snap| . Finish with basics if needed.



Room 3 - Build to 100% on anagamis/ arhats and  |bd|  +  |surge|  to south wall of sanctum's arena, then run to spot shown in picture (or mirrored if you chose other side): https://i.imgur.com/WaGAzLg.png
...
Room 1 - Head to the barrier on the bridge southeast of the room's entrance. As you will likely be on low adrenaline for this room, use mostly basics to kill 2x Cloaked Zealot. You should also drop 2  |dommine|  here. You want to be on high adrenaline when starting the next room. Be wary of your HP as the room contains a Hanto Sellsword, which can deal heavy damage.



Room 2 - Somewhere in the long north-south hallway, there will be 3 Death Lotus rogues. They will attack you with a very deadly Ranged attack. Block this by using  |devo|  +  |DeflectRange| . (remember to use  |devo|  before targetting anything to prevent adrenaline loss.) Use your  |adrenrenewalflask|  if it is off cooldown. Go to the 3x Cloaked Zealot and AoE them down(meteor strike here as clearing this barrier quickly is vital). Summoned Crassian Warriors should die nearly instantly, extending your  |devo| .



Room 3 - Run/ |surge| / |bd|  along the northern half of the circle walkway until you reach the next barrier. Barge a Waterfiend or Sea Crocodile towards the east side of the circle and kill the lone Cloaked Zealot in front of the barrier with a bled  |assault| + |cane| .





Room 4 - Head east, then north towards the 3x Cloaked Zealot in front of the next barrier. Kill them with  |gflurry|  and basics, building adrenaline towards  |zerk|  in preparation for the next room.



Room 5 - Head all the way east, up the stairs. If you aren't at 100% adrenaline, tag any nearby mob to build while moving.  |zerk|  as soon as you can and build adrenaline. Next to the door, there will be 6x Cloaked Zealot, 5 of them standing in a line and 1 right beside the barrier. Position yourself along the centre of the line and  |cane|  + basics until all 6 zealots are dead, then enter the door.

.. image:: https://i.imgur.com/tykiTgx.png
...
Room 1 - After leaving Sanctum's arena run south, east, them south again and  |surge|  after you go around corner. Keep running south and  |surge|  +  |bd|  around corner as soon as you can  |surge|  again (if low adren, grab a target before first  |surge|  and use a defensive). Run towards the bridge with 2 zealots, equip  |mechchin|  →  |needle|  →  |snipe|  →  |snap|  (if adren too low for  |snap| :  |bindingshot|  →  |corruptshot|  →  |snipe|  →  |piercing|  ).



Room 2 -  After killing the 2 zealots, continue south over the bridge, then east and  |surge|  as soon as you go around corner, keep running and  |bd|  +  |surge|  south after going around next corner. Equip  |mechchin|  and use basics/  |rapid|  to kill the 3 zealots ( |devo|  and  |DeflectRange|  if death lotus rogues use their spec). 



Room 3 - Use  |accel|  to get around the lake area (take north route). Use auto →  |snipe|  → q  |snap|  to kill the zealot guarding the gate.





Room 4 -   |surge|  east and run towards the next gate with 3 zealots. It's possible to  |bd|  diagonally across the bridge: https://i.imgur.com/9IQRudo.png

Using  |mechchin| , kill the 3 zealots with  |corruptshot| / |rapid| / basics while targetting the middle one (try to stand south, next to northern most zealot to be able to  |surge|  instantly onto strairs after killing them).



Room 5 -  |surge| /  |bd|  east, equip  |mechchin|  and target one of the underlined zealots: https://i.imgur.com/Sh696XE.png

 |snipe|  → q  |snap| , (on the other one you didnt target):  |corruptshot|  →  |shadowtend| . Use  |eofspec|   |dbow|  on the zealot next to the gate if high adren and finish whatever zealots are left with basics.
...
Room 1 -  |gbarge|  into either Elite Sakadagami, drop 2  |dommine|  and use  |assault| . Kill both Elite Sakadagami in front of the barrier.



Room 2 -  |surge|  down the stairs to get the attention of the 2x Eastern mercenary, then  |surge| + |bd|  northwest to get the attention of 4x Renegade Menaphite soldier. Kill the 6 afromentioned enemies with AoEs. Be wary of the Hanto sellswords in the room.



If you are low on food and HP, an alternative, safer method to clear this room is to kill the 2x Elite Sotapanna in room 1, then kill the 2x Eastern Mercenary before you  |surge| + |bd|  northwest and kill the 4x Renegade Menaphite soldier. This lets you avoid getting attacked by the dangerous Hanto sellswords.

`Example:` <https://streamable.com/klcaap>



Room 3 - This room contains two Death Lotus rogues(marked on map: yellow dots), which will not aggro on you when dealt with correctly.  |surge| + |bd|  through the central area of the room as is shown here

`Example:` <https://streamable.com/ryakvz>

If this is not done correctly, you will end up taking a lot of unnecessary damage and potentially even dying. After you have successfully  |bd|  into the west corridor, head up north and kill 4x Cloaked Zealot. Finish clearing the barrier with 100% adrenaline. Once the barrier is clear,  |freedom|  out of any bleeds you may have on you,  |surge|  to the bank chest, load your preset and kill Masuta, the Ascended.

.. image:: https://i.imgur.com/iW6qRt5.png
...
Room 1 - Use  |ricochet|  with long range weapon on one of the 2 sakadagamis, then step 2 tiles south + 1 tile west and manually run 2 tiles back towards them (https://i.imgur.com/tWzdnt5.png) (dont get pulled in by using an ability, otherwise the 2 sotapannas to west will start attacking you). Equip  |mechchin|  and kill them with basics/  |snap|  (using stuns on them will delay their stuns).



Room 2 -   |surge|  down the stairs and kill the 2 eastern mercenaries with basics/ thresholds (save  |rapid| ).  |bd|  north and stand on any of the tiles shown on picture: https://i.imgur.com/hoyklQH.png

If standing in white outline(1), target menaphite soldier #3; If in green outline(2), target menaphite soldier #2 or #3. (Make sure to not walk into them because this can mess up the lure. You can always walk east, however, as long as none of them are bound/stunned). Use  |corruptshot|  >  |rapid|  → when all of the soldiers are below 13k hp:  |snipe|  → q  |snap|  → finish with basics if needed.



Room 3 -   |surge| + |bd|  towards west gate. Target either of the 2 middle zealots /w  |mechchin|  and use  |corruptshot| / |needle|  >  |shadowtend|  >  |needle| /  |corruptshot| . Finish the rest with basics (dont use  |rapid| , otherwise it will be on cooldown for masuta). 

 |surge|  towards chest and bank.
...
.. image:: https://i.imgur.com/15ucDnm.png
...
-  |surge| + |bd|  on minimap onto west stairs (https://i.imgur.com/RXYeRnR.png), target middle pylon and use  |corruptshot|  →  |dazing|  +  |surge|  after releasing  |corruptshot|  >  |needle| > (either of the 2 south pylons):  |snipe|  > q  |mds|  →  |needle|  → snap the last pylon (basics if not 100% adren), get to 88%+ with defensive on the remaining pylons without attacking them.
...
.. image:: https://i.imgur.com/FPOOp1C.png





The first boss of the Temple of Aminishi follows a straightforward attack pattern containing highly damaging attacks which are capable of KOing someone unprepared. Once learned, this boss will essentially become a DPS dummy in where your killtime is reflective of your capability to deal raw damage. Stand to the north of the boss and DPS. Meleers should stand south, otherwise AoEs will not work ( |cleave| ,  |gflurry| ,  |vulnbomb| )
...
`Example clip of how to avoid the water jet with  |bd| :` <https://streamable.com/bj1ee1>
...
Start with  |zerk|  +  |adrenrenewal|  →  |gbarge|  →  |cleave|  →  |assault| (bled) →  |destroy|  →  |cleave|  →  |sever|  →  |gflurry|  → 188s.

Build with basics and use your  |zgs|   |spec|  while keeping track of boss HP and your adrenaline. Ideally you will get the boss's HP low enough to comfortably finish him off within your 2nd  |zerk| . Pay attention to both adrenaline gained and damage dealt, and decide on which abilities to use accordingly. Do not use  |assault|  here as should you use your 2nd  |zerk|  quickly,  |assault|  will not be off cooldown when needed.

Once you have 100% adrenaline after lowering the boss HP,  |zerk|  →  |gbarge|  → 188 → 188 →  |limitless|  +  |assault|  → 188 → 2-hit  |gflurry|  →  |destroy|  → basics until the boss is dead.
...
throw  |vulnbomb|  on south edge of sanctum's "pool" +  |DeathsSwift|  → auto →  |snipe|  → q  |needle|  →  |sgb|   |spec|  →  |limitless| + |rapid|  → dw  |snap|  →  |freedom|  → auto → dw  |snipe|  → q  |needle|  →  |shadowtend|  +  |disrupt|  →  |bindingshot| (flank)> 4t  |ecb|   |spec|  → auto →  |corruptshot|  →  |snipe|  → q  |rapid|  →  |snap|  → if  |eofspec|   |sgb| :  |sgb|  |spec|  →  |needle| , 

if normal  |sgb| :  |needle|  →  |sgb|   |spec|  →  |mds|  →  |snipe|  → q  |needle|  →  |bindingshot| (flank)> 4t  |ecb|   |spec|  → auto →  |corruptshot|  →  |rapid|  →  |snipe|  → q  |snap|  →  |eofspec|  |dbow| ( |mds|  if low) → improvise till finished

Use ruby at start of kill, switch to hydrix after proc, switch to hydrix after snap if no ruby proc and camp until you get enough adren to be able to use all thresholds in time in  |DeathsSwift|  (67% before  |ecb|  to be in the clear) use rubies after and switch to hydrix for 2 abilities after proc, camp hydrix after  |DeathsSwift|  until end of kill
...
.. image:: https://i.imgur.com/cUGy1Ad.png
...
Masuta's Hurricane attack lasts for about 12 seconds, hitting nearby players for increasing Melee damage once per tick. Masuta will chase after you at walking speed during this attack. The best way to deal with this is  |DeflectMelee|  +  |devo|  about 1 GCD after the attack starts. This allows you to continue dealing damage in MD and nullifies most of the damage you take. Note that you can flank Masuta from the north side during his Hurricane attack.
...
Phase 1:  |zerk|  +  |adrenrenewal|  →  |gbarge|  →  |cleave|  →  |assault|  →  |destroy|  →



- If Hurricane starts after 2 autos: Cancel  |destroy|  with  |deci|  +  |surge| / |bd|  away, stall  |cleave|  from out of distance -> run back in and release  |cleave|  with  |devo| . 

- If Hurricane starts later: Build with 188s. When Masuta starts his Hurricane, time  |devo|  appropriately.





Build with strong basics until  |zerk|  ends, after which use  |dismember|  →  |slaughter|  →  |zgs|   |spec|  → 188s until  |assault|  has 6 seconds of cooldown left, after which  |limitless|  →  |destroy|  →  |assault| . Finish the phase with strong basics. Remember to manage Masuta's Tsunami properly, even if it requires early cancellation of channeled abilities.



Flight phase: The waters die fairly quickly simply by using strong basics and whatever thresholds you have off cooldown. Be sure to utilise  |bd|  resets when moving from water to water.  |DBA|   |spec|  when ~70 seconds of the flight phase have passed, then build to 100% adrenaline with basics. It is recommended (though not necessary) to kill off any remaining waters before moving onto Masuta when phase 2 starts.





Phase 2:  |zerk|  +  |adrenrenewal|  →  |gbarge|  →  |cleave|  →  |assault|  →  |destroy|  → build with basics until  |zerk|  ends →  |dismember|  →  |slaughter|  →  |zgs|   |spec|  → build with basics and use strong thresholds. Masuta should die here. Note: don't use  |limitless|  during phase 2, as it should be saved for Seiryu.
...
It is recommended to take as much food as you can ( |grest| / |blueblubber|  because with how much you have to eat you would lose too much adrenaline/ brew down too much with solids/ sara brews) and because of this it's recommended to have a separate preset for masuta as banking would lose more time than you would save on a faster kill. 

Preset example: https://i.imgur.com/KMBXcjN.png

After banking step east so you are looking into masutas arena, use defensive and  |surge| + |bd|  onto stairs to masuta. Entire p1 should be ruby camp and equip hydrix for 2 abilities after ruby proc. 





Phase 1: Target cycle (tc is used to avoid missclicks) +  |ecb|   |spec|  →  |vulnbomb|  → 4t auto →  |rapid|  →  |snap|  →  |snipe|  → q  |frag|  →  |corruptshot|  →  |dazing|  →  |Sacrifice|  →  |needle|  →  |DeathsSwift|  →  |adrenrenewalflask|  + auto>  |snipe|  → q  |needle|  → 4t  |ecb|   |spec|  → auto →  |corruptshot|  →  |frag|  →  |rapid|  →  |snipe|  →  |snap|  →  |eofspec|  |dbow|  (use basics if low adren and  |eofspec|  |dbow|  asap). Improvise till phased (can use  |limitless|  to dump w/e thresholds are left because masuta doesn't always phase 

immediately once he reaches his threshold hp). Throw  |StormShards|  once masuta becomes invulnerable.

Note the time u phased on and add 1min20s to it (this will be when you use  |nat|  on geysers)

Surge away when masuta starts his spin spec and keep kiting until your  |ecb|   |spec|  runs out (should be at  |Sacrifice|  in your rotation). Eat to full while kiting. If masuta does late spin, use  |bd| /  |surge|  to get far enough away to not have to leave your  |DeathsSwift| . 
...
Camp onyx bolts; use  |devo| / |res|  in downtime; Try to  |bd|  directly into geysers so when you kill them, your  |bd|  comes off cooldown; Try to md geysers when you're killing them to gain damage reduction from masutas mage attack; Stack 2 more  |StormShards|  for a total of 3 stacks;  |nat|  on p1 phase time+ 1min20s and equip hydrix bolts.
...
Phase 2:  (camp ruby and switch to hydric for 2 abils after proc until ~130k  hp, camp hydrix after that)
 ...
 |DeathsSwift|  → auto +  |vulnbomb|  →  |snipe|  → q  |needle|  → 4t  |ecb|  |spec|  → auto  |corruptshot|  →  |frag|  →  |snipe|  → q  |rapid|  +  |adrenrenewalflask|  when it comes off cooldown>  |snap|  →  |eofspec|  |dbow|  (skip for basic if low adren)> improvise basics till  |ecb|  |spec|  ends →  |needle|  →  |smoketend|  →  |Shatter|  →  |ecb|  |spec|  → improvise basics>  |rapid|  →  |snap|  → improv till finished.
...
.. image:: https://i.imgur.com/1LxKntU.png
...
When the huge breath attack starts, the game message "Seiryu takes a huge breath" will appear on your screen. This attack is an accurate, high-damaging (~8k) Magic attack. You can dodge this attack by standing within 3 tiles of the southern edge of the arena, you can block it with Disruption shield/Resonance or you can simply tank it.



When Seiryu's shadow energy attack starts, the game message "Shadow energy bursts forth from Seiryu." appears, and arrows will appear under your character, on the arena and on your minimap pointing to a specific tile. Seiryu will launch a shadow projectile towards this specific tile. Once the shadow projectile lands, highly damaging black hands will move from the marked tile towards your current location. Be sure to dodge these hands by sidestepping out of their path.
...
Start the fight with a  |zerk|  →  |gbarge|  → 188 → 188 →  |limitless|  +  |assault|  ->  |destroy|  → build with strong basics until Zerk ends →  |dismember|  →  |zgs|   |spec| . Optimally when Seiryu reaches 7.2M HP, you will have 100% adrenaline ready for the crystal phase. With this in mind, decide on which/how many thresholds to use depending on how much damage you've dealt to the boss. If your damage output is high, prioritise  |gflurry|  to ensure  |zerk|  is off cooldown for the crystal phase.
...
When Seiryu phases at 7.2M HP, swiftly move to either side's jump spot (marked here: <https://i.imgur.com/hcc69l5.png>) and click the jump option on the shackle platform. After the climbing animation:
 ...
 |zerk|  +  |adrenrenewal|  →  |gbarge|  + drop  |dommine|  → thresholds+basics until the first crystal is dead.



Before the wave of heals reaches the crystals, be sure to have used a fully channeled  |gflurry|  twice and to be on 100% adrenaline.  |nat|  before the first enigma reaches Seiryu and build to 100%, then, either 1 or 2 ticks after the first Uber heal hitsplat appears:

 |zerk|  →  |dismember|  → 2x Defensive(not  |freedom| ) →  |gbarge|  + drop  |dommine|  →  |cleave|  →  |assault|  →  |destroy|  → 3 hit  |gflurry|  →  |deci|  →  |freedom| . (Example clip: https://streamable.com/h2y1qo) You will be teleported down after  |deci| .





If the second crystal was destroyed, you can ignore the enigmas and attack Seiryu:  |zgs|   |spec|  → basics →  |destroy|  →  |gflurry|  → build to 100%.

If not,  |ricochet|  with your Caroming switch on the 3rd or 4th enigma to make all of them attack you. Move up to Seiryu and  |zgs|  |spec|  → basics until the enigmas are gathered around you →  |cane|  →  |quake|  →  |gflurry|  → build to 100%.



After reaching 100%,  |zerk|  →  |gbarge|  →  |backhand|  → 188 →  |limitless|  +  |assault|  →  |destroy|  → build to 50% →  |gflurry|  → strong abilities until Seiryu is phased. You will not have  |zerk|  available immediately for the second crystal phase. If the second crystal was not destroyed, jump up from the same side as you did earlier and use whatever abilities you deem necessary to finish it off. Ideally, when the second crystal is destroyed, you have 100% adrenaline and both  |zerk|  and  |gbarge|  off cooldown. For the final crystal,  |zerk|  +  |adrenrenewal|  →  |gbarge|  + Drop  |dommine|  → 188s and thresholds until the third crystal is destroyed.
...
*Note:* These rotations assume you are using 3T  |snipe| 



(Camp  |HydrixBakri| )

Use a defensive then  |surge|   |bd|  during gcd

Auto → DW  |corruptshot|  →  |DeathsSwift|  → Auto +  |snipe|  (with flanking offhand) →  |bindingshot|  →  |limitless|   |rapid|  → DW  |snap|  →  |freedom|  AA → DW  |snipe|  →  |needle|  →  |shadowtend|  →  |bindingshot|  → 4T  |ecb|  |spec|  → Auto +  |snipe|  →  |corruptshot|  →  |rapid|  →  |snap|  →  |sgb|   |eofor|  ( |bindingshot|  if you dont have sgb amulet) →  |snipe|  (with flanking offhand) →  |bindingshot|  → Improvise till phase





P1 notes:  |DeathsSwift|  further away from head (east/west) so u can dodge Seiryu's tendril attack without leaving flank distance, lower hp to ~5k before shadow tendrils, only heal up to ~3k before each mage hit after shadow tendrils, camp mage prayer after  |ecb|  |spec|  spec to lower your hp even more (~1.5k).



**Crystal 1** (Camp  |RubyBakri| , swap to  |HydrixBakri|  if you get a ruby proc)

 |vulnbomb|  + Auto +  |nip|  +  |dommine|  →  |ecb|  |spec|  →  |snipe|  →  |corruptshot|  →  |rapid|  →  |mds|  +  |stickybomb|  → DW  |piercing|  →  |HydrixBakri|   |DeathsSwift|  +  |stickybomb|  → Auto +  |snipe|  →  |piercing|  →  |freedom|  AA →  |snap|  +  |stickybomb|  →  |piercing|  →  |needle|  → Improvise till crystal is finished





**Crystal 2** (Camp  |HydrixBakri| , swap to  |RubyBakri|  if you get to 90%+ before  |dbow|   |eofspec| , camp  |HydrixBakri|  after  |dbow|   |eofspec| )

 |vulnbomb|  +  |ecb|  |spec|  +  |nip|  → Auto +  |corruptshot|  +  |stickybomb|  →  |snipe|  →  |piercing|  →  |rapid|  →  |HydrixBakri|   |dbow|   |eofspec|  +  |stickybomb|  →  |piercing|  →  |stickybomb|  +  |snipe|  →  |piercing|  →  |mds|  /  |snap|  → Improvise till crystal is finished





**Crystal 3** (Camp  |RubyBakri| , swap to  |OnyxBakri|  if you get a ruby proc)

Build to 100% before the 1st heal splat →  |vulnbomb|  +  |nip|  → 2 ticks after 2nd heal  |needle|  →  |ecb|  |spec|  (Place  |dommine|  when cooldown ends)  →  |snipe|  →  |corruptshot|  →  |rapid|  +  |adrenrenewal|  (around ~6-7th tick of rpaid fire)  →  |snap|  →  |OnyxBakri|   |dbow|   |eofspec|  →  |snipe|  →  |dbow|   |eofspec| 
...
- This is not the best way to go for a 1-cycle as a "one time thing", but rather a way to finish the dungeon quickly to boost runs/hour.
...
**Crystal 1**
 ...
 |zerk|  +  |adrenrenewal|  +  |vulnbomb|  →  |gbarge|  +  |dommine|  →  |cleave|  →  |assault|  →  |destroy|  →  |deci|  →  |gflurry| /188s until crystal 1 dies



**Crystal 2**

 |vulnbomb|  + 188/ |gflurry|  → 188s until Zerk runs out →  |dismember|  →  |bloodtendrils|  → 188s → out of distance  |zgs|  |spec|  → Defensive →  |gbarge|  → 188s →  |assault| * →  |destroy|  → 188s/thresholds according to HP/adrenaline, kill crystal 2 before it gets healed and build to 100% adrenaline.
...
If you have a  |mwspear| ,  |dismember|  before Zerk.



1 tick after the first heal appears,  |zerk|  → Defensive →  |gbarge|  →  |cleave| (Stall) → release  |cleave|  +  |sever|  +  |dommine|  →  |limitless|  +  |assault|  → 3hit  |gflurry|  →  |destroy|  →  |deci|  →  |cleave| 
