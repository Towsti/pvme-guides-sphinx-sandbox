...
.. |spices| image:: https://cdn.discordapp.com/emojis/662038182807732255.png?v=1
    :width: 1.375em
    :height: 1.375em
...
.. |jas| image:: https://cdn.discordapp.com/emojis/662032366667300875.png?v=1
    :width: 1.375em
    :height: 1.375em
...
`General path during the kill` https://i.imgur.com/1mJzYy4.jpg
...
`Ice surge` https://i.imgur.com/pcrHhwf.jpg

`Ice run` https://i.imgur.com/ho1EVDX.png
...
The blue square is where the 2/4 will stand in the start of the kill, they will also  |vulnbomb|  from here
...
.. image:: https://i.imgur.com/SZEwb5k.jpg
...
-  |jas|  **Make sure to use jas boost**
...
`1/3` Tag with  |frag|  → Auto +  |mds|  →  |snipe|  →  |stw|  (  |mds|  or  |rout|  if needed)

`2/4` Tag with  |mds|  → Auto +  |snipe|  →  |mds|  →  |stw|  (  |mds|  or  |rout|  if needed)



**Umbra**

`1/3`  |corruptshot|  →  |needle| 
...
Tag with  |snap|  →  |Vuln|  →  |snipe|  →  |bombard|  →  |mds| 
...
`1/3` Tag with  |frag|  →  |shadowtend|  →  |snap|  →  |mds| 

`2/4` Tag with  |mds|  →  |shadowtend|  →  |snap|  →  |mds| 
...
`1/3` Tag with  |mds|  →  |deadshot|  →  |frag|  →  |limitless|  →  |rapid| 

`2/4` Tag with  |corruptshot|  →  |onsl| 
...
-  |jas|  **Make sure to use jas boost**
 ...
 |dummy|  → Build adren if needed →  |needle|  →  |ecb|   |spec|  →  |anti| 
...
`1/3` Tag with  |frag|  →  |ecb|   |spec|  + Auto →  |snap|  ( |mds|  if needed)

`2/4` Tag with  |corruptshot|  → Auto +  |snipe|  →  |mds|  ( |Tuskas|   if needed)
...
`1/3` Tag with  |dbow|   |eofspec|  →  |freedom|  +  |vulnbomb|  → Any defensive for adren →  |snipe|  →  |mds| 

`2/4` Tag with  |snap|  →  |freedom|  →  |ecb|   |spec|  →  |snipe|  →  |mds| 



**Cruor**

`1/3`  |dommine|  →  |ecb|   |spec|  →  |ricochet| 

`2/4`  |dommine|  →  |corruptshot|  →  |ecb|   |spec| 
...
`1/3`  |anti|  during phase transition → Auto +  |snap|  →  |mds|  

`2/4`  |anti|  during phase transition →  |shadowtend|  →  |mds|  



**Glacies**

`1/3`  |corruptshot|  →  |ecb|   |spec|  ( |Tuskas|   if needed)

`2/4`  |frag|  →  |ecb|   |spec|  ( |ricochet|   if needed)
...
`1/3` Tag with  |frag|  →  |dbow|   |eofspec|  →  |limitless|   |rapid|   →  |Tuskas|  /  |rout|  if needed

`2/4` Tag with  |corruptshot|  →  |dbow|   |eofspec|  →  |limitless|   |rapid|  →  |Tuskas|  /  |rout|  if needed





Camp  |RubyBakri| ,  |HydrixBakri|  on minions and after blood,  |OnyxBakri|  Zaros
...
`1/3` <https://youtu.be/76x_NobujNQ>

`2/4` <https://youtu.be/htDi9dnqKVQ>
